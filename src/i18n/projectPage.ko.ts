...
const openClawKo: OpenClawContent = {
  ...
  role: '프로젝트 제작자 · 시스템 설계 · 에이전트 운영 · 프로덕트 오너 · 런타임은 직접 구현',
  stats: [
...
const caseStudiesKo: Record<string, CaseStudyContent> = {
  'morphus-website': {
    ...
    before: [
      ...
      'AI는 "아마 될 겁니다"로 넘기곤 했습니다. 코드가 그럴듯하게 읽힌다는 이유로 완료라고 보고하고, 실제로 검증하지는 않았습니다.',
    ],
    after: [
  ...
  'persona-workflows': {
    ...
    problem: '사람 버튜버는 아무래도 피로, 스케줄, 기억, 일관성에 묶일 수밖에 없습니다. 과제는 AI 캐릭터를 "진짜 살아있는 것처럼" 느끼게 하면서도, 실제 라이브 스트리밍 운영을 견딜 만큼 안정적으로 만드는 것이었습니다. 가장 어려웠던 건 대본을 읽는 게 아니라 방송 아젠다를 생성하는 일이었습니다. 초기 아젠다는 지나치게 정형적이고 AI 티가 나서 캐릭터의 말투가 아니었고, 여러 차례 고쳐서야 지금의 흐름 — 캐릭터에서 출발해 주제를 찾고, 아젠다를 만들고, 대본을 전달하고, 채팅을 읽고, 필요하면 대본을 벗어나 즉흥으로 가는 — 에 도달했습니다.',
    before: [
      ...
      '방송 아젠다 생성은 처음엔 지나치게 정형적이고 AI 티가 났습니다. 그 캐릭터가 할 법한 말이 아니었습니다.',
    ],
    after: [
      '지금은 4개의 AI 버튜버 캐릭터가 실제로 활동하고 있습니다 — Dearu, NaisKuri, AkuCat, Amber. 모두 회사 소속이고, 제가 설계를 주도해 동료들과 함께 만들었습니다.',
      '시스템은 실제 시청자로부터 3000시간 넘는 시청 시간을 쌓았습니다. 라이브 스트리밍, 영상, 쇼츠를 합산한 누적치입니다.',
      'AI 캐릭터가 안정적인 상태로 완전 자동 라이브 스트리밍을 진행할 수 있게 됐습니다. 아젠다는 캐릭터 말투로 생성되고, 채팅에 반응하고, 필요할 때는 대본을 벗어나 즉흥으로 갑니다.',
      'Gamania(遊戲橘子)의 버추얼 탤런트 「宮祈緣」과 협업해, 풀 AI 버전을 정식 공개하기 전에 A/B 테스트를 진행했습니다. 사람이 연기한 쪽과 AI가 실연한 쪽을 비교했고, 양측 모두 AI 쪽이 더 낫다는 결론에 동의했습니다.',
    ],
    contributions: [
      '페르소나, 런타임, 라이브 워크플로우 전반의 프로젝트 관리와 실행을 주도했습니다.',
      '캐릭터의 페르소나와 동작 방식을 설계했습니다. 가치관과 취향부터 행동 규범까지, "AI가 무엇이든 할 수 있어서는 안 된다"는 캐릭터의 경계선을 정밀하게 그었습니다.',
      '런타임 이슈와 스트리밍 플로우 안정성을 디버깅하고, AI 티가 너무 났던 아젠다 생성 흐름을 다시 설계했습니다.',
      '캐릭터 설정, 툴 연결, 자동 쇼 진행 동작을 정의했습니다.',
      '데모에도 쓸 수 있고 실제 운영에 가까운 라이브 퍼포먼스가 나오도록 시스템을 조율했습니다.',
    ],
    outcomes: [
      { title: '실제 시청자로 검증', detail: '실험실 데모가 아닙니다. Dearu의 첫 방송에서는 우연히 들어온 시청자들이 끝까지 AI라는 걸 알아채지 못했습니다.' },
      { title: '멈추지 않는 공연', detail: '사람 공연자와 달리 AI 캐릭터는 쉬지 않고 스트리밍하고, 스크립트도 정확히 지키고, 시청자 맥락까지 기억합니다.' },
      { title: '상업 협업', detail: 'Gamania와 진행한 「宮祈緣」 A/B 테스트에서 AI 버전이 사람이 연기한 쪽보다 낫다는 결론이 나왔습니다. 기술은 라이선스로 제공했고, 이후 캐릭터 운영은 상대 쪽 판단입니다.' },
    ],
  ...
  'portfolio-site': {
    ...
    before: [
      ...
      '영어판 contact 라벨은 중국어판보다 깁니다. 에이전트가 중국어판 줄바꿈만 확인한 탓에 영어판은 375px에서 깨진 채 남아 있었고, 제가 가장 좁은 화면의 실기기 확인을 요구하고 나서야 발견됐습니다.',
    ],
    after: [
