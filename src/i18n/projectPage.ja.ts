...
const openClawJa: OpenClawContent = {
  ...
  role: 'プロジェクト発起人 · システム設計 · エージェント運用 · プロダクトオーナー · ランタイムは自分で実装',
  stats: [
...
const caseStudiesJa: Record<string, CaseStudyContent> = {
  'morphus-website': {
    ...
    before: [
      ...
      'AIは「たぶん動く」で済ませがちでした。コードがそれらしく読めるだけで完了と報告し、実際には検証していません。',
    ],
    after: [
  ...
  'persona-workflows': {
    ...
    problem: '人間のVtuberは、どうしても疲労やスケジュール、記憶、一貫性に縛られます。課題は、AIキャラクターを「ちゃんと生きている」ように感じさせつつ、実際のライブ配信運用に耐えるだけの安定性を持たせることでした。いちばん難しかったのは台本を読み上げることではなく、配信アジェンダの生成です。初期のアジェンダは定型的でAIらしさが強く、キャラクターの口調になりませんでした。何度も作り直して、いまの流れ — キャラクターから出発して話題を見つけ、アジェンダを生成し、台本を届け、コメントを読み、必要なら台本を外れて即興する — にたどり着いています。',
    before: [
      ...
      '配信アジェンダの生成は当初、定型的でAIらしさが強く、そのキャラクターが言いそうな言葉になっていませんでした。',
    ],
    after: [
      '今は4体のAI Vtuberが実際に活動しています — Dearu、NaisKuri、AkuCat、Amber。いずれも会社所属で、私が設計を主導し、同僚と一緒に立ち上げました。',
      'システムは実際の視聴者から、3000時間以上の視聴時間を積み上げました。配信・動画・ショートの累計です。',
      'AIキャラクターが、安定したまま完全自動でライブ配信をこなせるようになりました。アジェンダはキャラクターの口調で生成され、コメントに反応し、必要なときは台本を外れて即興します。',
      'Gamania（遊戲橘子）のバーチャルタレント「宮祈緣」との協業では、フルAI版を正式公開する前にA/Bテストを実施しました。人間による演技とAIによる実演を比較し、双方ともAIのほうが良かったという結論で一致しました。',
    ],
    contributions: [
      'ペルソナ・ランタイム・ライブワークフロー全体のプロジェクト管理と実行を主導しました。',
      'キャラクターのペルソナと振る舞いを設計しました。価値観や好み、行動規範まで、「AIは何でもできる存在であってはならない」という人格の境界を精密に引いています。',
      'ランタイムの不具合や配信フローの安定性をデバッグし、AIらしさが強すぎたアジェンダ生成の流れを再設計しました。',
      'キャラクター設定・ツール連携・自動進行の挙動を決めました。',
      'デモにも使えて、本番に近いライブ配信パフォーマンスになるよう、システムを調整しました。',
    ],
    outcomes: [
      { title: '実際の視聴者で検証済み', detail: '研究室のデモではありません。Dearuの初配信では、たまたま入ってきた視聴者が、最後までAIだと気づかないまま見ていました。' },
      { title: '止まらないパフォーマンス', detail: '人間の出演者と違って、AIキャラクターはずっと配信し続けられて、台本も正確に守り、視聴者の文脈もちゃんと覚えています。' },
      { title: '商業案件での協業', detail: 'Gamaniaとの「宮祈緣」A/Bテストでは、AI版のほうが人間による演技より良いという結論になりました。技術はライセンス提供し、その後キャラクターをどう運用するかは先方の判断です。' },
    ],
  ...
  'portfolio-site': {
    ...
    before: [
      ...
      '英語版のcontactラベルは中国語版より長いのですが、エージェントは中国語版の折り返ししか確認していませんでした。そのため英語版は375pxで崩れたまま残り、最も狭い画面での実機確認を私が求めるまで見つかりませんでした。',
    ],
    after: [
